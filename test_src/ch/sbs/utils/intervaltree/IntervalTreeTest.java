...
public class IntervalTreeTest {
	...
	public void testEmptyCase() {
		assertEquals(null, tree.search(new Interval(0, 42)));
	}
	...
	public void testSimpleOverlapCaseOnLeft() {
		tree.insert(new Interval(1, 5));
		assertEquals(new Interval(1, 5), searchInterval(0, 3));
	}

	@Test
	public void testSimpleOverlapCaseOnRight() {
		tree.insert(new Interval(1, 5));
		assertEquals(new Interval(1, 5), searchInterval(3, 6));
	}

	@Test
	public void testSimpleOverlapCaseOnEnclosed() {
		tree.insert(new Interval(1, 5));
		assertEquals(new Interval(1, 5), searchInterval(2, 3));
	}

	@Test
	public void testSimpleOverlapCaseOnSurrounding() {
		tree.insert(new Interval(1, 5));
		assertEquals(new Interval(1, 5), searchInterval(0, 6));
	}

	@Test
	public void testSimpleUnsuccessfulMatchOnRight() {
		tree.insert(new Interval(1, 5));
		assertEquals(null, searchInterval(6, 10));
	}

	@Test
	public void testSimpleUnsuccessfulMatchOnLeft() {
		tree.insert(new Interval(2, 5));
		assertEquals(null, searchInterval(0, 1));
	}
	...
	private List<Interval> searchAllIntervals(int low, int high) {
		final List<Interval> intervals = tree
				.searchAll(new Interval(low, high));
		sort(intervals);
		return intervals;
	}
	...
	public void testExpectedSizeWithDuplicates() {
		...
		final int someNumber = 20;
		for (int i = 0; i < someNumber; i++) {
			prepareTestCaseTree();
		}
		assertEquals(n * (1 + someNumber), tree.size());
	}
	...
	public void testSearchAllOnWholeResults() {
		...
		assertEquals("unexpected unequality", expected,
				searchAllIntervals(0, bignumber - 1));

		assertEquals("unexpected unequality", expected,
				searchAllIntervals(0, 2 * bignumber));

		assertEquals("unexpected unequality", expected,
				searchAllIntervals(0, 3 * bignumber));

		assertEquals("unexpected unequality", new ArrayList<Object>(),
				searchAllIntervals(-1, -1));
	}
	...
	public void testWithLotsOfOverlapping() {
		...
		for (int i = 0; i < bignumber; i++) {
			assertEquals(i, searchAllIntervals(bignumber - i, bignumber).size());
		}
	}
}
